...
\subsection{Bayesian probabilistic analysis}
...
\caption{Geographical clustering results  for a lowest-BIC spherical covariance Gaussian Mixture Model of the stations located inside the red circle in Figure \ref{shortlist}b. (a) BIC results for different number of clusters. (b) Lowest-BIC GMM geographical clustering classification. Marker size is proportional to the probability $P_j$, where the largest size corresponds to  $Q_j=10$ and the smallest size corresponds to $Q_j=2.2$. }\label{geobic}
\end{center}
\end{figure}
...
Figure \ref{geobic}b shows 6 candidate regions of interest, where weather stations indicating dry, sunny climate are regionally correlated. In order to visualize the extent of those regions we plotted the predicted variance regions for each cluster along with other (rejected) stations in our sample in Figure \ref{latlonreg}. From this figure, we can reject regions where the number of rejected stations within the predicted variance regions is too similar to the number of stations in our shortlist for a specified candidate region in Figure \ref{geobic}b. We discuss the suitability of each of these regions below, from South to North.

\begin{figure}
\begin{center}
\includegraphics[scale=0.55]{latlonreg.pdf}
\caption{Geographical clustering results  for the lowest-BIC spherical covariance Gaussian Mixture Model shown in Figure \ref{geobic}b, indicating the predicted variance regions for each cluster (red circles), and plotted along rejected stations in our original dataset (black points).}\label{latlonreg}
\end{center}
\end{figure}
