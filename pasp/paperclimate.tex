...
\caption{Climate clusters as predicted by the lowest-BIC Gaussian Mixture Model for precipitation. Left column: Average monthly precipitation and standard deviation (error bars) for each climate cluster. Right column: Geographical location and altitude of weather stations in each precipitation cluster.}
\end{center}
\end{figure}
...
\caption{Climate clusters as predicted by the lowest-BIC Gaussian Mixture Model for rain days. Left column: Average monthly rain days and standard deviation (error bars) for each climate cluster. Right column: Geographical location and altitude of weather stations in each rain days cluster.}\label{clustd}
\end{center}
\end{figure}
...
\caption{Climate clusters as predicted by the lowest-BIC Gaussian Mixture Model for rain days. Left column: Average monthly rain days and standard deviation (error bars) for each climate cluster. Right column: Geographical location and altitude of weather stations in each rain days cluster.}
\end{center}
\end{figure}
...
\caption{Climate clusters as predicted by the lowest-BIC Gaussian Mixture Model for sunshine duration. Left column: Average monthly sunshine duration and standard deviation (error bars) for each climate cluster. Right column: Geographical location and altitude of weather stations in each sunshine duration cluster.}\label{clustb}
\end{center}
\end{figure}
...
We quantify the quality of a given site using the probability for the hypothesis that a station which appears in our shortlist meets all four criteria, given that it is located at a given elevation. We can write down this probability in terms of the following events:
...
\begin{itemize}
\item $C$ is actually the conjunction of all four of the $R,D,H,S$ events ``The station belongs to a preferred cluster for variable $X$'', i.e. $C=R\cap D\cap H \cap S$.
\end{itemize}
...
Since the probabilities for stations in our shortlist vary by orders of magnitude, for clarity's sake we decided to use a probability-based logarithmic quality index for all stations in our shortlist. For $j=(0,1,...,N_\mathrm{stations})$,
\begin{equation}
...
\caption{Map of stations in our shortlist. The highlighted region on (a) is seen in more detail in (b), which corresponds approximately to the Colombian \emph{altiplano} region. Marker size is proportional to the probability $P_j$, where the largest size corresponds to $Q_j=10$.}\label{shortlist}
\end{center}
\end{figure}



Given that many of our 83 candidate stations were lacking in relative humidity and sunshine duration data, we extrapolated using data from nearby stations (at less than 4 km away and at an elevation difference of less than 100 m). We made sure that in general, for stations less than 5 km away, the difference in relative humidity and sunshine duration data is less than 15\%. The extrapolated data evidenced that 4 of the stations in the shortlist did not satisfy the relative humidity and sunshine duration criteria.  The remaining 79 stations are mapped in Figure \ref{shortlist}a.\\

We rejected 9 of the stations in Figure \ref{shortlist}a, i.e. those located outside of the region located within the latitude, longitude range $[(4.3,6.2)^\circ,(-72.4,-74.5)^\circ]$. Even tough they could be indicative of unusually dry, sunny regions, there are simply not enough stations nearby to say anything conclusively, as evidenced in Figure \ref{outliers}. Adding to this, the only variable they measured was precipitation ($Q_j=2.2$), which leads us to reject them from our shortlist. \\
...
Figure \ref{unif} shows 6 candidate regions of interest, where weather stations indicating dry, sunny climate are regionally correlated. In order to visualize the extent of those regions we plotted the predicted variance regions for each cluster along with other (rejected) stations in our sample in Figure \ref{unif}. It should be noted that the spherical variance GMM geographical clustering overestimates the extent of each region, and tells us nothing of their actual shape. We can now reject regions where the number of rejected stations within the predicted variance regions is too similar to the number of stations in our shortlist in Figure \ref{unif}.  We discuss the suitability of each of these regions below, from South-West to North-East based on our Bayesian probabilistic quality index (Section \ref{bpa}).
...
\begin{table}
\caption{\label{tabfin}Regions of interest with a high-altitude daytime dry, clear-sky climate, organized by geographical cluster number ($N_C$), station location, Bayesian probability quality measure ($Q_j$), distance to cluster center ($r_c$), mean precipitation  ($R$), mean rain days ($D$), mean relative humidity  ($h$), mean sunshine duration  ($S$), and altitude ($h$). Detailed table (including all clusters) available at \texttt{https://github.com/saint-germain/ideam} \\}

\begin{tabular}{rlrrrrrrr}
\toprule
 $N_C$  &                 Municipality  &  $Q_j$ &  $r_c$  &  Mean $R$ &  Mean $D$  &  Mean $H$  &  Mean $S$ &  $h$ \\
...
From our shortlist of 79 stations that met our criteria, we selected 70 stations located  in the Cundinamarca-Boyaca \emph{altiplano} region of Colombia, which is a high-mountain basin with an average altitude of 2600 masl. Weather stations in our shortlist but outside this region are too sparsely located to suggest a candidate region of interest. \\
...
We summarize the information about two regions of interest (Valle de Ubat\'e and Valle del Sol) in Table \ref{tabfin}, selected based on a Bayesian probabilistic quality index. Most of the stations in this table are located at elevations above 2600 masl and below 3000 masl. The upper bound for precipitable water vapor given by  \citeasnoun{suen2014} and \citeasnoun{suen2016} for these regions is $\sim15$ mm, which means that it is not implausible to find a site with an even lower pwv column in one of the regions of interest identified here. For instance, there are some mountains nearby Valle de Ubat\'e and Valle del Sol which have altitudes of up to 3400 masl, which are sites that warrant further measurements using radio sondes and radiometers. \\
