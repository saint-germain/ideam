...
# CAMBIO: In OSX, use gcut instead of cut, as the OSX version of cut doesn’t like —option-delimiter
grep -A 45 'VALORES MEDIOS  DIARIOS DE HUMEDAD' IDEAM.txt | grep MEDIA | gcut -c24-25,33-34,42-43,51-52,60-61,69-70,78-79,87-88,96-97,105-106,114-115,123-124 --output-delimiter=' ,' | tr -d '\015' | sed 's/\*/ /g' > temp.txt
...
# CAMBIO: Count nr. of fields < 12 for each line in temp, write to fields.txt
awk '{if (NF<12) print (12-NF)*4-1;else print 0 }' temp.txt > fields.txt
...
grep -A2 'VALORES MEDIOS  DIARIOS DE HUMEDAD' IDEAM.txt | grep ANO | awk '{print $7,$10}' > temptag.txt
