...
import sys
a=sys.argv[1]
cmd = "grep -A 55 'MEDIOS  DIARIOS DE HUMEDAD' IDEAM.txt | grep -A55 'ANO  {0}' | grep -A2 'VALORES  ANUALES' | grep MEDIA | awk '{{print $2}}' > {0}mean.txt".format(a)
p = subprocess.Popen(cmd , shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
p.wait()
cmd = "grep -A 60 'MEDIOS  DIARIOS DE HUMEDAD' IDEAM.txt | grep -A4 'ANO  {0}' | grep ELEVACION | awk '{{print $2}}' > {0}alt.txt".format(a)
p = subprocess.Popen(cmd , shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
p.wait()
